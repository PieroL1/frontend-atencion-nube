    ...
    <div className="bg-white rounded-lg border border-gray-200 p-5 hover:shadow-md transition-shadow">
        ...
        <div className="flex items-center gap-2">
          <span
            className={`px-3 py-1 rounded-md text-xs font-medium border ${
              TIPOS[tutoria.type] || "bg-gray-50 text-gray-700"
            }`}
          >
            {tutoria.type}
          </span>
          {tutoria.code && <span className="text-xs text-slate">#{tutoria.code}</span>}
        </div>
        <span
          className={`px-2.5 py-1 rounded-full text-xs font-medium ${
            ESTADOS[tutoria.state] || "bg-gray-100 text-gray-600"
          }`}
        >
          {tutoria.state}
        </span>
      ...
      <div className="space-y-2 mb-4">
        <div className="flex items-center text-sm text-ink">
          <svg
            className="w-4 h-4 mr-2 text-slate"
            fill="none"
            stroke="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div className="space-y-2">
        ...
        {acciones.puedeCancelar && (
          <button
            ...
            disabled={loading}
            className="w-full py-2 px-4 border border-red-300 text-red-700 rounded-lg hover:bg-red-50 text-sm font-medium transition-colors disabled:opacity-50"
          >
            ✕ Cancelar
          </button>
        )}
