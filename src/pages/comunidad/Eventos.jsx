  ...
  return (
    <>
      ...
      <div className="max-w-7xl mx-auto p-6">
        ...
        <div>
          <h1 className="text-3xl font-bold text-ink dark:text-slate mb-2">Eventos Comunitarios</h1>
      ...
      {showForm && (
        <div className="bg-white dark:bg-night rounded-lg shadow-sm dark:shadow-slate/10 p-6 mb-6">
          <h2 className="text-xl font-semibold text-ink dark:text-slate mb-4">Nuevo Evento</h2>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              ...
              <textarea
                ...
                rows={4}
                className="w-full px-4 py-2 border border-gray-300 dark:border-slate/30 bg-white dark:bg-night/50 text-ink dark:text-slate rounded-lg focus:ring-2 focus:ring-primary focus:border-transparent resize-none"
                maxLength={200}
                required
              />
              <p className="text-xs text-slate dark:text-slate/70 mt-1">
            ...
            <div className="flex justify-end gap-2 pt-2">
              <button
                ...
                className="px-6 py-2 border border-gray-300 dark:border-slate/30 text-ink dark:text-slate rounded-lg hover:bg-gray-50 dark:hover:bg-night/50 transition"
                disabled={creating}
              >
                Cancelar
              </button>
              <button
                type="submit"
                disabled={creating}
                className="px-6 py-2 bg-green-600 dark:bg-green-700 text-white rounded-lg hover:bg-green-700 dark:hover:bg-green-800 disabled:bg-gray-400 dark:disabled:bg-gray-600 disabled:cursor-not-allowed transition"
              >
      ...
      {/* Filtros */}
      <form onSubmit={handleFilter} className="bg-white dark:bg-night rounded-lg shadow-sm dark:shadow-slate/10 p-4 mb-6">
      ...
      {loading ? (
        <div className="text-center py-12">
          ...
          <p className="text-slate dark:text-slate/70 mt-4">Cargando eventos...</p>
        </div>
      ) : eventos.length === 0 ? (
        <div className="text-center py-12 bg-white dark:bg-night rounded-lg shadow-sm dark:shadow-slate/10">
          <span className="text-6xl mb-4 block">📅</span>
          <p className="text-slate dark:text-slate/70 text-lg mb-4">No se encontraron eventos</p>
          {!showForm && (
            <button
        ...
        <>
          ...
          {upcomingEvents.length > 0 && (
            <div className="mb-8">
              <h2 className="text-2xl font-bold text-ink dark:text-slate mb-4 flex items-center gap-2">
                <span>🎯</span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {upcomingEvents.map((evento) => (
                  <div
                    ...
                    className="bg-white dark:bg-night rounded-lg shadow-sm dark:shadow-slate/10 hover:shadow-md dark:hover:shadow-slate/20 transition-all p-6 cursor-pointer hover:scale-105"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="text-lg font-semibold text-ink dark:text-slate flex-1 line-clamp-2">
                        {evento.titulo}
                      </h3>
                      <span className="text-2xl ml-2">🎉</span>
                    </div>

                    <div className="flex items-center text-sm text-primary mb-3 font-medium">
                    ...
                    <p className="text-slate dark:text-slate/70 text-sm line-clamp-3">
                      {evento.description}
                    </p>

                    {evento.creator_name && (
                      <div className="flex items-center text-xs text-slate dark:text-slate/70 pt-3 border-t border-gray-200 dark:border-slate/20 mt-3">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {pastEvents.map((evento) => (
                  <div
                    key={evento.id}
                    onClick={() => handleEventoClick(evento)}
                    className="bg-gray-50 dark:bg-night/50 rounded-lg shadow-sm dark:shadow-slate/10 p-6 opacity-75 cursor-pointer hover:opacity-90 hover:shadow-md dark:hover:shadow-slate/20 transition-all"
                  >
                    <div className="flex items-start justify-between mb-3">
                      <h3 className="text-lg font-semibold text-slate dark:text-slate/80 flex-1 line-clamp-2">
                        {evento.titulo}
                      </h3>
                      <span className="text-2xl ml-2 grayscale">🎉</span>
                    </div>

                    <div className="flex items-center text-sm text-slate dark:text-slate/70 mb-3">
                      <svg className="w-4 h-4 mr-1" fill="currentColor" viewBox="0 0 20 20">
                        <path fillRule="evenodd" d="M6 2a1 1 0 00-1 1v1H4a2 2 0 00-2 2v10a2 2 0 002 2h12a2 2 0 002-2V6a2 2 0 00-2-2h-1V3a1 1 0 10-2 0v1H7V3a1 1 0 00-1-1zm0 5a1 1 0 000 2h8a1 1 0 100-2H6z" clipRule="evenodd" />
                      </svg>
                      {formatEventDate(evento.event_date)}
